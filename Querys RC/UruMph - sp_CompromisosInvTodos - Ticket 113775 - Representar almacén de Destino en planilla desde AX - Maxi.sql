...
declare
 @AREA				varchar(04),						-- Empresa de Ejecución (QUITAR EL OPCIONAL = NULL!!!)
 @MODELO_PSP		varchar(10),						-- Modelo de PSP (QUITAR EL OPCIONAL = NULL!!!)
 @ALMACEN_GRUPO		varchar(500) = null,				-- Opcion para agrupar almacenes. Si el almacen existe se especifica sino Otros. No debe filtrar datos, solo agrupar.
 @ALMACEN_GESTION	varchar(30)	 = null,				-- Gestion Directa (solo 1 se debe definir un unico almacen)
 @ConVMI			varchar(01)  = null,				-- Opcional por Roemmers UY (para indicar si se muestra o no InvVMI)	0=No	1=Sí
 @Salida		varchar(05)  = null					-- Opcional, por defecto es EXCEL (EXCEL, BD)
...
-- ***************************************************************************************************************************************************************************************
--
--	FUM:	10/02/2020 SCa+		Se modificac script para que puede generar tabla en otra BD (COMPRAS-MLe)
--  FUM:	10/07/2020 RC		Se agrega id Diario recepción para transacciones estado Registrado. Ticket P.Valla 98985	
--
-- ***************************************************************************************************************************************************************************************

--- EJEMPLO DE COMO SE EJECUTA --------------------------------------------------------
/*
exec dbo.sp_INV_CompromisosTodos '100', '_PREVISION'
exec dbo.sp_INV_CompromisosTodos '120', 'E-Logistic'
exec dbo.sp_INV_CompromisosTodos '010', '_Prevision', 'PR0501,PRM501,PRV501,PRReserva', 'PR0501'
exec dbo.sp_INV_CompromisosTodos '010', '_Prevision', 'PR0501,CM0101', 'PR0501', 1
exec dbo.sp_INV_CompromisosTodos '120', '_Prevision', NULL,  NULL, 0, 'EXCEL'
exec dbo.sp_INV_CompromisosTodos '120', '_Prevision', NULL,  NULL, 0, 'BD'
exec dbo.sp_INV_CompromisosTodos '120', null, null, null
exec dbo.sp_INV_CompromisosTodos '120', 'E-Logistic', null, null
exec dbo.sp_INV_CompromisosTodos '120', 'eLogistica', null, null
exec dbo.sp_INV_CompromisosTodos '120', '_PREVISION', null, null
exec dbo.sp_INV_CompromisosTodos '120', '_ANALISIS', null, null
*/
----------------------------------------------------------------------------------------------------

/*  Habitlitar estas línes para ejecutar la consulta sin crear el SP
Declare
...
--*/
  ...
  set @AREA			= isnull(@AREA,'120')									-- QUITAR ESTE OPCIONAL SOLO PARA PRUEBA !!!
  set @MODELO_PSP	= isnull(@MODELO_PSP,'')								-- QUITAR ESTE OPCIONAL SOLO PARA PRUEBA !!!  
...
-- 30/04/2019	SCa-	-- Se agrega Marca de Empaque
  ...
  having sum(Qty) != 0
  	...
  	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 2', 'Fin', getutcdate()  

	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 3', 'Ini', getutcdate()
...
-- 30/04/2019	SCa-	-- Se agrega Marca de Empaque
  ...
  having sum(Qty) != 0
  	...
  	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 3', 'Fin', getutcdate()  

	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 4', 'Ini', getutcdate()
...
-- 30/04/2019	SCa-	-- Se agrega Marca de Empaque
  ...
  having sum(Qty) != 0
  	...
  	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 4', 'Fin', getutcdate()  


	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 5', 'Ini', getutcdate()
...
-- 30/04/2019	SCa-	-- Se agrega Marca de Empaque
  ...
  having sum(Qty) != 0
  	...
  	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 5', 'Fin', getutcdate()  

	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 6', 'Ini', getutcdate()
...
-- 30/04/2019	SCa-	-- Se agrega Marca de Empaque
  ...
  having sum(Qty) != 0
  	...
  	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 6', 'Fin', getutcdate()  

	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 7', 'Ini', getutcdate()
  ...
  where	fs.DataAreaID = @AREA and
	...
	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 7', 'Fin', getutcdate()  

	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 8', 'Ini', getutcdate()
...
-- 30/04/2019	SCa-	-- Se agrega Marca de Empaque
  ...
  having sum(Qty) != 0
  	...
  	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 8', 'Fin', getutcdate()  

	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 9', 'Ini', getutcdate()
  ...
  where t.DataAreaID = @AREA and
	exists (select 1
 	...
 	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 9', 'Fin', getutcdate()  

	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 10', 'Ini', getutcdate()
  ...
  where t.DataAreaID = @AREA
	...
	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 10', 'Fin', getutcdate()  

	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 11', 'Ini', getutcdate()
  ...
  where
	...
	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 11', 'Fin', getutcdate()  

	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 12', 'Ini', getutcdate()
  ...
  where 
	...
	round(((pl.QtyOrdered-pl.RemainInventPhysical) / pl.QtyOrdered) * 100, 2) >= 90							-- Linea cumplida en un 90%
		...
		insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 12', 'Fin', getutcdate()  

	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 13', 'Ini', getutcdate()
  ...
  where 
	...
	t.TransType in (2,8,9,10) 																				-- Solo Transacciones de "Produccion" como se definio en el PASO 4
		...
		insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 13', 'Fin', getutcdate()  

	insert into dbo.XLS_INV_CompromisosTodos_ControlPerf
	select 'Paso 14', 'Ini', getutcdate()
...
--			f.TTMov																														as [TrnRefTTMov],	
			...
			, isnull( (select top 1 id2.INVENTLOCATIONID  
			from inventtrans itr 
			...
			where (itr.DATEPHYSICAL = '1900-01-01' 
...
--			and t.Qty != 0	
		and a.ArtGrupoCod in ('PT', 'PTTR', 'MMTR')
		and	itr.DATAAREAID = t.DataAreaID 
			and itr.ITEMID = t.ItemID
			), '')																											AS Destino 
		...
		left join VendTable vt on
			vt.ACCOUNTNUM = a.ProvPrinArtCod
			and vt.DATAAREAID = t.DataAreaID
		where t.DataAreaID = @AREA and
	--end
  --drop table #TEMP
